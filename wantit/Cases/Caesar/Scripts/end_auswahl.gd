...
signal EndDialogue(DialogueStart: String)
...
func _ready() -> void:
	...
	await get_tree().process_frame #Warten weil die Größe erst nach dem ersten Frame gesetzt 
	%Button.position.x -= %Button.size.x/2

func _on_button_pressed() -> void:
	EndDialogue.emit("Ende" + str(Ende))
